public class GradeBook {
	...
	System.out.println("Lowest grade and highest grade are " + getMinimum() + " " + getMaximum());
	outputBarChart();
}
	public int getMinimum() {
	int lowGrade = grades[0];
	for(int grade : grades) if (grade < lowGrade) lowGrade = grade;
	return lowGrade;
}

	public int getMaximum() {
	int highGrade = grades[0];
	for(int grade : grades) if (grade > highGrade) highGrade = grade;
	return highGrade;
}
